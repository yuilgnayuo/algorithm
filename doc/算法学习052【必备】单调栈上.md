...
<img src="images/Weixin Image_20251231080400_57_145.jpg" alt="Monotonic-stack" style="zoom:50%;" />
...
public class MonotonicStack {
    ...
    private static void compute() {
        ...
        // 跳过n-1位置，因为它一定是正确值
        // 只更新右侧的值，因为左侧也一定是正确的
        // 主意：什么情况答案不对，是值相同的时候，它被迫出栈的
        // 也就是 3->3的右侧的值保存的是6->3，因此他们一定相同的时候，3->3位置右侧的值才需要被更新
        for (int i = n - 2; i >= 0; i--) {
...
> http://leetcode.cn/problems/largest-rectangle-in-histogram/

思路：当前以 i 位置为长方形的高，往左，右开始扩，能扩到的最大长度 * 当前高，那么就是最大面积。
...
 **思路：**转换成**以 i 行为底**的可以组成的最大矩形的面积。然后求解过程和上题一模一样。

**主意：**当 i 行任意位置为0，那么它的这个位置就是0，否则才考虑跟它上一行组成的大高度。

``` java
int MAX = 201; // 1 <= rows, cols <= 200
int[] stack = new int[MAX];
